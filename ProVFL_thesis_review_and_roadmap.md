...
***Round 9 update (2026-09-11) — Multi-Dataset Generalization & Pipeline Hardening.**
1. **Pandas 2.0+ Object Array Type Coercion Bug Resolved**: In pandas $\ge$ 2.0, `pd.get_dummies()` outputs boolean (`bool`) columns by default. When concatenated with normalized continuous float columns and label categories, calling `.to_numpy()` without an explicit dtype yielded `numpy.object_` 2D arrays, causing PyTorch tensor conversion crashes (`TypeError: can't convert np.ndarray of type numpy.object_`) on Bank Marketing (`bankmk`) and Census (`census`). Fixed across `datasets/bankmk.py`, `datasets/census.py`, `datasets/lawschool.py` with `dtype=float` in `pd.get_dummies()` and explicit `dtype=np.float32`/`np.int64`, plus fully vectorized, type-safe tensor conversion in `my_utils/utils.py:tensor_data_create`.
2. **Pre-flight & Unit Test Hardening**: Gate [4/5] in `kaggle_setup.py` now executes `dataloader.get_data()` to verify live single-batch data loading and tensor dimensions for all configured datasets (`adult`, `bankmk`, `census`) before burning any GPU hours. Added `test_tensor_data_create` to `test_defense_func.py`.
3. **Multi-Dataset Generalization Execution**: Launched 2 parallel Kaggle background sessions:
   - **Part 1**: `s1c` re-run (with fixed `test_ab_correlation.py` exporting `ab_feat_correlation.csv` and `raw_baseline` output) + `s1g` on Adult (`property=race`, `race_White` at ~85.5%, 9 jobs, ~2.2 h).
   - **Part 2**: `s1g` on Bank Marketing (`property=marital`, `marital_married` at ~60.2%, 9 jobs, ~3.2 h).
...
| 31 | **Pandas 2.0+ `bool` upcast yielded `numpy.object_` 2D arrays, crashing PyTorch tensor conversion** | ✅ **Fixed (Round 9)** | In pandas $\ge$ 2.0, `pd.get_dummies()` produces boolean columns by default. When concatenated with float columns, calling `.to_numpy()` defaulted to `object_` arrays. PyTorch's `torch.FloatTensor` constructor threw `TypeError: can't convert np.ndarray of type numpy.object_`, failing all 9 jobs on Bank Marketing (`bankmk`). Fixed with `dtype=float` in `pd.get_dummies()`, `.astype(float)` on target property series, and explicit `dtype=np.float32`/`np.int64` in `.to_numpy()` across `datasets/bankmk.py`, `datasets/census.py`, and `datasets/lawschool.py` |
| 32 | **Slow list comprehension in `tensor_data_create` replaced with vectorized PyTorch conversion** | ✅ **Optimized (Round 9)** | `my_utils/utils.py:tensor_data_create` previously used `[torch.FloatTensor(i) for i in features]` (O(N) Python objects). Replaced with vectorized `np.asarray(features, dtype=np.float32)` and `torch.from_numpy(...)`, eliminating overhead and guaranteeing type safety |
| 33 | **Setup Gate [4/5] only checked file existence, not runtime dataset batch construction** | ✅ **Enhanced (Round 9)** | `kaggle_setup.py` gate [4/5] now calls `dataloader.get_data()` to generate a live mini-batch ($x, y, p$) for every dataset, asserting tensor shape and dtype before any training job is launched |
| 34 | **CPU unit test suite expanded** | ✅ **Added (Round 9)** | `test_defense_func.py` now includes `test_tensor_data_create`, testing conversion of mixed `numpy.object_` arrays and random floats to PyTorch tensors (<0.01s on CPU) |
| 35 | **Multi-dataset generalization execution (`s1g`) launched across parallel background sessions** | 🚀 **Running (Round 9)** | Part 1 (`s1c` re-run + `s1g adult/race`, 9 jobs, ~2.2 h) and Part 2 (`s1g bankmk/marital`, 9 jobs, ~3.2 h) executing concurrently on Kaggle T4 GPUs |
...
| **P2.5 — confound + constructive ceiling** | **`--session s1c`** | **13 / 2.9 h** | 🚀 **Running (re-run with `feat_corr` fix)** | 12/13 passed in Round 8; re-running full suite with relative-path fix on `test_ab_correlation.py` to extract `ab_feat_correlation.csv` alongside `raw_baseline` |
| **P2.6 — generalisation** | **`--session s1g --dataset <d> --property <p>`** | **adult-race 2.2 h; bankmk 3.2 h; census 17.3 h → `--part`** | 🚀 **Running (2 parallel sessions)** | Replicates the *decomposition* across domains: Part 1 runs `adult/race` (~2.2 h); Part 2 runs `bankmk/marital` (~3.2 h, after pandas 2.0 type fix). Clears the ≥3-setting bar |
