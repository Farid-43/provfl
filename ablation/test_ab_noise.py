...
def main(args):  
    ...
    for noise in [0, 1, 2, 5, 10, 15, 20, 25, 30]:
        _nlen = int(len(aux_prop_index) * (noise / 100.0))
        new_prop_index = np.concatenate((aux_prop_index[_nlen:], aux_nonprop_index[:_nlen]))
        new_nonprop_index = np.concatenate((aux_nonprop_index[_nlen:], aux_prop_index[:_nlen]))
        vfl_info['noise'] = noise

        clf = 'XGB'
        args.attack_feat = 'b_grad'
        property_single(vfl_info, clf, args, features['b_grad'], new_prop_index, new_nonprop_index, prop_npy, file_name)
        args.attack_feat = 'b_output'
        property_single(vfl_info, clf, args, features['b_output'], new_prop_index, new_nonprop_index, prop_npy, file_name)
        args.attack_feat = 'a_grad'
        property_single(vfl_info, clf, args, features['a_grad'], new_prop_index, new_nonprop_index, prop_npy, file_name)
        args.attack_feat =  'a_output'
        property_single(vfl_info, clf, args, features['a_output'], new_prop_index, new_nonprop_index, prop_npy, file_name)

        args.attack_feat = 'all'
        property_ensemble(vfl_info, 'XGB', args, features, new_prop_index, new_nonprop_index, prop_npy, file_name)
