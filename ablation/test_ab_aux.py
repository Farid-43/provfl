...
def main(args):
    ...
    for aux_size in [1000, 2000, 3000, 4000, 5000, 6000]:
        args.sampling_size = aux_size

    
        clf = 'XGB'
        args.attack_feat = 'b_grad'
        property_single(vfl_info, clf, args, features['b_grad'], aux_prop_index[:aux_size], aux_nonprop_index[:aux_size], prop_npy, file_name)
        args.attack_feat = 'b_output'
        property_single(vfl_info, clf, args, features['b_output'], aux_prop_index[:aux_size], aux_nonprop_index[:aux_size], prop_npy, file_name)
        args.attack_feat = 'a_grad'
        property_single(vfl_info, clf, args, features['a_grad'], aux_prop_index[:aux_size], aux_nonprop_index[:aux_size], prop_npy, file_name)
        args.attack_feat =  'a_output'
        property_single(vfl_info, clf, args, features['a_output'], aux_prop_index[:aux_size], aux_nonprop_index[:aux_size], prop_npy, file_name)

        args.attack_feat = 'all'
        property_ensemble(vfl_info, 'XGB', args, features, aux_prop_index[:aux_size], aux_nonprop_index[:aux_size], prop_npy, file_name)
