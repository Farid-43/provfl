...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (trn_X, trn_y, prop_label) in enumerate(train_loader):
            ...
            # Use DEFENDED gradients (z_gradients_*_clone) -- this reflects what
            # an attacker actually observes in VFL (the active party sends defended
            # gradients to passive parties). Outputs remain original since they are
            # generated by each party's own bottom model before communication.
            if batch_idx == 0:
